...
\begin{figure}[ht]
  ...
  \begin{scaletikzpicturetowidth}{\linewidth}
    \begin{tikzpicture}[squarednode/.style={rectangle, draw=black}]
      \node[squarednode,fill=red!30] (view) {View};
      \node[squarednode,fill=blue!30] (presenter) [right=of view] {Presenter};
      \node[squarednode,fill=orange!30] (model) [right=of presenter] {Model};
      \node[squarednode,fill=green!30] (listener) [above=of presenter] {Sensor Listeners};
      \node[squarednode,fill=purple!30] (database) [above=of model] {Database};
...
\subsection{Altitude Calculation}\label{subsec:altitude-calc-design}
  ...
  \begin{scaletikzpicturetowidth}{0.3 * \linewidth}
    \begin{tikzpicture}[scale=\tikzscale]
      \begin{scope}[very thick,
        ...
        % Wind
        \draw[blue,->] (2,-7.5)--(2,-5) node[black,align=center,midway,left]{Wind\\Direction};
      \end{scope}
    \end{tikzpicture}
  \end{scaletikzpicturetowidth}
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}
    \tikzstyle{entity}=[rectangle,rectangle split,rectangle split parts=2,draw=black,text width=6.5cm]

    \node (Jump) [entity] {\textbf{Jump}
        \nodepart{second}
          id: integer (\textbf{Primary Key}) \\
          time: integer
        };

    \node(Position) [entity,below=2cm of Jump] {\textbf{Position}
        \nodepart{second}
          id: integer (\textbf{Primary Key}) \\
          useruuid: text \\
          altitude: integer \\
          ...
          time: integer \\
          fall\_type: integer \\
          jump\_id: integer (\textbf{Foreign Key})
        };

      \draw[->] (Position)--(Jump) node[at end,left, yshift=-2mm]{1} node[at start,left,yshift=2mm]{0..N} node[midway,right]{jump\_id:id};
  \end{tikzpicture}
  \end{scaletikzpicturetowidth}
  \caption{Database entity-relationship model}\label{fig:database-relations}
\end{figure}

% Database structure/relation model
Figure~\ref{fig:database-relations} shows the entity-relationship model of the database, displaying primary and foreign keys and all of the possible entry values.
